		...
		ardupin_t * arduidiot)
{
	...
	float axis_pp_per_mm[4] = DEFAULT_AXIS_STEPS_PER_UNIT;	// from Marlin!
	{
		...
		stepper_init(avr, &r->step_x, "X", X_ENABLE_ON, axis_pp_per_mm[0], 0, 200, 0);
	...
	}
	{
		...
		stepper_init(avr, &r->step_y, "Y", Y_ENABLE_ON, axis_pp_per_mm[1], 1, 200, 0);
	...
	}
	{
		...
		stepper_init(avr, &r->step_z, "Z", Z_ENABLE_ON, axis_pp_per_mm[2], 0, 190, 0);
...
//	snprintf(avr_flash_path, sizeof(avr_flash_path), "%s/%s", pwd, "simduino_flash.bin");
	...
	elf_firmware_t f;
	const char * fname = "marlin/build/marlin.elf";
